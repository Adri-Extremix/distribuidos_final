...
Los requisitos de la aplicación nos dificultan el desarrollo de pruebas de manera reproducible, por lo que se ha optado por realizar las pruebas en local y analizar los resultados.
Estas pruebas se realizarán a través de la ejecución de varios procesos en distintos terminales para la correcta visualización de las distintas impresiones.

Al principio de cada prueba se iniciarán el servidor, el servidor RPC y el servidor web; y al final de estás se cerrarán estos servidores.
...
Para probar el correcto funcionamiento de la comunicación entre clientes se tendrán dos clientes que publiquen archivos y obtenga el archivo del otro.
...
\begin{center}
    \begin{lstlisting}[caption=Operaciones a realizar por el cliente A]
        ...
        GET_FILE B /tmp/ficheroB.txt /tmp/fileB.txt
    \end{lstlisting}
\end{center}
\begin{center}
    \begin{lstlisting}[caption=Operaciones a realizar por el cliente B]
        REGISTER B
        CONNECT B
        PUBLISH /tmp/ficheroB.txt HOLA MUNDO
